...
if (require.main === module) {
  ...
  assert.equal(catcher.G.aiming, true, 'A moving orb can be caught');
  ...
  assert.equal(catcher.G.shots, 2, 'A mid-flight relaunch uses one launch');
  assert(catcher.orb.vy < -2 && Math.abs(catcher.orb.vx) < 0.001, 'Relaunch changes flight direction');
  catcher.events.pointerdown(pointer(cx + 20, cy));
  ...
  assert.equal(catcher.G.shots, 2);
  catcher.events.pointerdown(pointer(cx, cy));
  catcher.events.pointerup(pointer(cx, cy));
  assert.equal(catcher.G.shots, 2, 'Tapping without dragging must not spend a launch');
  assert.equal(catcher.orb.vy, resumeVy);
  ...
  assert.equal(catcher.G.aiming, false, 'Catching cannot bypass the launch budget');
  const danger = boot();
  danger.go(0);
